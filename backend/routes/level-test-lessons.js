...
router.get('/:language', async (req, res) => {
  try {
    ...
    const lessonsData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../data/level-test-lessons.json'), 'utf8')
    );
    ...
    console.log(`Загружены тестовые уроки (${language}):`, lessons.length);
    
    res.json({ lessons });
  } catch (error) {
    console.error('Ошибка получения тестовых уроков:', error);
